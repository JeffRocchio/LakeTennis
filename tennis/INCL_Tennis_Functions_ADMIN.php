<?php
/*
	1/21/2007. This version of this include file is 'frozen.'
I am currently in the process of migrating to v2 of this file.
No changes should be made to this file. As migration occurs
the include statements that call this file should be changed
so they call the v2 version of this file.
...
function ADMIN_GenFieldText(&$fldLabel, &$fldHelp, $fldAuth, $fldFrmName, $fldMaxLen, $fldDispLen, &$fldValue)
	{
	/*
		This function generates the HTML for 1 row of a table
		that is being used in a data-entry form
	...
	*/
	...
	$rowHTML .= "</TD>{$CRLF}";
	$rowHTML .= "<TD class=deTblCellInput><P class=deFieldInput>";
	if (ADMIN_EditAuthorized($fldAuth))
		{
...
function ADMIN_GenFieldYN(&$fldLabel, &$fldHelp, $fldAuth, $fldFrmName, $fldValue)
	{
	/*
		   This function generates the HTML for 1 row of a table
		that is being used in a data-entry form
...
function ADMIN_GenFieldNote(&$fldLabel, &$fldHelp, $fldAuth, $fldFrmName, $fldRows, $fldCols, $fldValue)
	{
	/*
		This function generates the HTML for 1 row of a table
		that is being used in a data-entry form
...
function ADMIN_GenFieldDropCode(&$fldLabel, &$fldHelp, $fldAuth, $fldFrmName, $cdSet, $fldValue, $shtName)
	{
	/*
		   This function generates the HTML for 1 row of a table
		that is being used in a data-entry form.
...
function ADMIN_GenFieldDropTbl(&$fldLabel, &$fldHelp, $fldAuth, $fldFrmName, $tblName, $fltrID, $fldValue, $shtName)
	{
	/*
		   This function generates the HTML for 1 row of a table
		that is being used in a data-entry form
...
function Tennis_dbRecordInsert(&$post, $tblName)
	{
	/*
	...
	ASSUMES:
		1) Mysql connection is currently open.
		2) The new data is contained in the global $_POST array,
	...
	*/
	...
	if ($DEBUG)
		{
		echo "<p>Insert Query:<BR>{$query}</p>";
		}
	...
	if (!$qryResult)
		...
		return $message;
				...
				//   Report results.
	$message = "<p>Records Added: {$qryResult}</p>";
	if ($DEBUG)
		{
		$message .= '<P>Whole query: ' . $query . "</P>";
		}
...
function ADMIN_dbGetNameTbl($tblName, $recID, $shtName)
	...
	if ($DEBUG)
		{
		echo "<p>in ADMIN_dbGetNameTbl()</p>";
		echo "<p>QUERY: {$query}</p>";
		}
	$qryResult = mysql_query($query);
	if (!$qryResult)
...
function ADMIN_EditAuthorized($fldAuth)
	{
...
function ADMIN_Post_HeaderOK($tblName, $rtnPage, &$message)
	{
