<?php
/*
	12/20/2015: Note the KLUDGE in local_GenSection_rsvpPostEdits().
	
	This script allows a person to edit their RSVP record for an
	event at which participants are expected to bring something
	(e.g., the Mixed-Up Doubles events).
	
	11/23/2014: Version 1.0.
...
function local_fetchData()
	...
	if (!$rsvpID)
		{
		echo "<P>ERROR, No RSVP Selected.</P>{$CRLF}";
		include '../INCL_footer.php';
		$qryResult = false;
		exit;
		}
...
/*
	KLUDGE: For my convience, when it is me who is viewing the
page I am displaying the login hash key. This is so I can supply the
login URL to folks individually when they need it (vs having to explain
to them their ID/Password and how to login 'manually').
*/
	if ($userPrivSeries == "ADM")
		{
		$field1 = $rsvpPrsnID + 10000;
		$field2 = 43 + 10000;
		$field3 = $eventID + 10000;
		$keyAsString = $field1 . $field2 . $field3;
		$url2Pg = "http://".$_SERVER['HTTP_HOST'] . "/tennis/editRSVPviaEmailMixedDbls.php?x=" . $keyAsString;
		$logInLink = "<P>*** Auto Login URL:<BR />" . $url2Pg . "</P>";
		echo $logInLink;
		}
	...
	echo "<form method='post' action='meditRSVPPrsnMixedDbls.php?ID={$rsvpID}&POST=T'>{$CRLF}";
