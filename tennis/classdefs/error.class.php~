...
.
   */
	...
	//---------------------------------------------------------------------------
	public function getErrCount($reportedClass="UNREPORTED")
	{
	/*	PURPOSE: Get the number of errors in the list.

		ASSUMES --:

		TAKES --:
				a) Error reported status to return count for.

		RETURNS --:
				a) The number of errors currently registered..
	*/

	switch ($reportedClass)
		{
		case "UNREPORTED":
			return $this->errsUnreported;
			break;
			
		default:
			return $this->errsInQueu;
		}
